    ...
    def __init__(self, current_amp, rx, ry, rz, sigma_ex, origin=None, morphology=None, frequency=None,
        ...
        if morphology is None:
            ...
            if node_length is None or internode_length is None or paranode_length is None:
                raise InvalidInput("Please initialize a value for node length, paranode length, and internode length")
        ...
        if sigma_ex <= 0:
            raise InvalidInput("Conductivity values should be greater than zero")
        if self.sine_wave:
            self.frequency, self.pw, self.duty_cycle = frequency, None, None
            if frequency <= 0:
                raise InvalidInput("Frequency should be greater than zero")
        else:
            self.frequency, self.pw, self.duty_cycle = None, pulse_width, duty_cycle
            if pulse_width <= 0:
                raise InvalidInput("The pulse width should be greater than zero")
